...
To illustrate the process we will use an abstract example. Let's imagine that we have a bag full of balls. Each balls represents a gene in the *gene universe*. 
...
**Question:** Look back at the instructions to filter the DESeq2 results from the previous section. How would you modify the filtering condition to filter the file `degust.tsv`.
...
<div class="alert alert-info">
If you have problems importing the file `gsea_input.rnk`, check (by loading the file into a text editor) that the 2nd column does not contain the Word `Tumour` in it. If so, delete this row of data
